  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="售后状态" align="center" prop="aftermarketStatus">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.aftermarket_status" :value="scope.row.aftermarketStatus"/>
        </template>
      </el-table-column>
        ...
        <template slot-scope="{row}">
          ...
          <div>
            <el-button
              size="mini"
              type="text"
              @click="handleAftermarketFollow(row,'detail')"
              v-hasPermi="['order:after:followup']"
            >
              <i>
                <svg-icon class-name="svg-abnormal" icon-class="flag"/>
              </i>
    ...
    <a-modal
      centered
      :title="title"
      :visible.sync="openAftermarketStatus"
      width="50%" :destroyOnClose="true"
      append-to-body @cancel="openAftermarketStatus = false">
      <el-form
        :model="aftermarketStatusForm"
        class="edit"
        ref="aftermarketform"
        label-width="90px"
        >
        <el-row>
          ...
          <el-form-item label="售后状态" prop="aftermarketStatus">
            <el-select v-model="aftermarketStatusForm.aftermarketStatus">
              <el-option
                v-for="dict in dict.type.aftermarket_status"
                :key="dict.value"
                :label="dict.label"
          ...
          <el-col :span="24">
            <el-form-item label="备注" prop="aftermarketStatusDesc">
              <el-input
                type="textarea"
                :autosize="{minRows: 2, maxRows: 3}"
                v-model="aftermarketStatusForm.aftermarketStatusDesc"
                placeholder="请输入备注说明"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <div class="flex flex-justify-end mr20">
              <el-button type="primary" size="mini" @click="submitAftermarketForm">提 交</el-button>
            </div>
          </el-col>
        </el-row>
      </el-form>

      <el-divider content-position="center">售后状态变更记录</el-divider>
      <el-table
        :data="aftermarketFollowupList"
        border
      >
        <el-table-column label="订单状态" prop="aftermarketStatus" align="center">

        </el-table-column>
        <el-table-column label="提交人" prop="createName" align="center">

        </el-table-column>
        <el-table-column label="备注" prop="aftermarketStatusDesc" show-overflow-tooltip align="center">
        </el-table-column>
      </el-table>
...
import {
  ...
  updateInfo,
  addAbnormal,
  abnormalFollowupList, addAbnormalFollowup, rollbackAbnorma, aftermarketFollowupList, addAftermarketFollowup
...
export default {
  ...
  data() {
    return {
      ...
      aftermarketStatusForm: {
        aftermarketStatus: '',  // 售后状态
        aftermarketStatusDesc: '',  // 备注
        orderId: '',  // 订单id
      },
  ...
  methods: {
    ...
    // 获取售后状态变更记录列表，打开售后状态框
    handleAftermarketFollow(row) {
      const {id: orderId} = row
      this.aftermarketStatusForm.orderId = orderId
      this.aftermarketStatusForm.aftermarketStatus = row.aftermarketStatus
      aftermarketFollowupList({orderId}).then(res => {
        this.aftermarketFollowupList = res.data
        this.openAftermarketStatus = true
        console.log(this.aftermarketFollowupList)
        console.log(this.aftermarketStatusForm.aftermarketStatus)
      })
    },
    /* 更新售后状态 */
    submitAftermarketForm() {
      addAftermarketFollowup(this.aftermarketStatusForm).then(res => {
        this.$modal.msgSuccess("提交成功");
        this.resetAftermarketStatusForm()
        this.getList()
        this.openAftermarketStatus = false
      })
    },
    ...
    getList() {
      this.loading = true;
      listAftermarketInfo(this.queryParams).then(response => {
    ...
    resetAftermarketStatusForm() {
      this.aftermarketStatusForm = {
        orderId: '',
        aftermarketStatus: '',
        aftermarketStatusDesc: '',
      }
    },
