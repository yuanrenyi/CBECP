...
<!--        <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4.8">
        ...
        <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
          <el-form-item label="订单新增时间" prop="addTime">
            <div class="flex flex-justify-between">
              <el-date-picker clearable
                              v-model="queryParams.addTimeBegin"
                              type="datetime"
                              value-format="yyyy-MM-dd HH:mm:ss"
                              placeholder="请选择订单新增起始时间"
                              style="width: 30%">
              </el-date-picker>
              <span v-html="'&nbsp;&nbsp;-&nbsp;&nbsp;'"></span>
              <el-date-picker clearable
                              v-model="queryParams.addTimeEnd"
                              type="datetime"
                              value-format="yyyy-MM-dd HH:mm:ss"
                              placeholder="请选择订单新增结束时间"
                              style="width: 30%" >
              </el-date-picker>
            </div>
          </el-form-item>
    ...
    <el-table
      ...
      <el-table-column v-if="showSupplier" label="供应商" align="center" prop="supplierName"/>
    ...
    <a-modal :title="title" :visible.sync="open" width="65%" append-to-body @cancel="cancel">
      ...
      <el-form
        ...
        <el-row :gutter="10">
          ...
          <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="6" v-if="showSupplier">
            <el-form-item label="供应商" prop="supplierId">
...
<!--          <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="6">
          ...
          <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="6">
            <el-form-item label="销售方可提现金额" prop="sellerWithdrawAmount">
              <el-input v-model="form.sellerWithdrawalAmount" placeholder="请输入销售方可提现金额"/>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="6">
            <el-form-item label="供应商可提现金额" prop="supplierWithdrawalAmount">
              <el-input v-model="form.supplierWithdrawalAmount" placeholder="请输入销售方可提现金额"/>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      ...
      <el-collapse v-else v-model="activeNames">
        <el-collapse-item title="订单基本信息" name="1">
          <el-descriptions
            ...
            <el-descriptions-item v-if="showSupplier">
              <template slot="label">
                供应商
              </template>
              {{ form.supplierName }}
        ...
        <el-collapse-item title="商品及支付信息" name="2">
          <el-descriptions
            :labelStyle="{width: '110px'}"
            :contentStyle="{width: '220px'}"
...
-->
            <el-descriptions-item>
              ...
              <dict-tag :options="dict.type.order_pay_mode" :value="form.payMode"/>
            </el-descriptions-item>
        ...
        <el-collapse-item title="其他信息" name="4">
          <el-descriptions
            ...
            <el-descriptions-item>
             ...
             <!-- <span>{{ parseTime(form.callbackTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>-->
              {{ form.callbackTime }}
            </el-descriptions-item>
            <el-descriptions-item>
              <template slot="label">
...
import {uploadFile, uploadFileAndResult} from "@/utils/request";
export default {
  name: "Info",
  ...
  methods: {
    checkPermi, checkRole, uncheckRole,
    ...
    handleUpdate(row) {
      ...
      getOrderGoodsList(id).then( resp => {
        this.orderGoodsList = resp.data
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      let optIds = [];
      this.ids.forEach(order=>{
        optIds.push(order.id);
      });
      console.log(optIds)
      this.$modal.confirm('是否确认删除订单信息编号为"' + optIds + '"的数据项？').then(function () {
        return delInfo(optIds);
      }).then(() => {
        this.getList();
    ...
    handlePush(row) {
      ...
      let outTradeNos = [];
      this.ids.forEach(order=>{
        outTradeNos.push(order.outTradeNo);
      });
      console.log(outTradeNos)
      this.$modal.confirm('是否推送订单编号为"' + outTradeNos + '"的数据项？').then(function () {
        let data = {'outTradeNos':outTradeNos};
        return pushInfo(data);
      }).then((response) => {
        if(response.code === 200) {
          this.$modal.msgSuccess("推单成功");
        } else {
    ...
    uploadZipFile (file) {
      let url = '/system/order/upload'
      uploadFileAndResult(url, 'orderFile',file,{
        timeout: 300 * 1000
      }).then(res=>{
        console.log(res)
