import {listSeller} from "@/api/seller/seller";
...
export default {
  created() {
    // 是管理员或供应商则查询
    if(this.checkRole(['admin', 'SUPPLIER_ADMIN'])) {
      this.getSellerList()
    }
  },
  data() {
  ...
  methods : {
    ...
    * */
    sellerFormChange (val,vModel,idParam,nameParam) {
      if(val.id) {
        if(idParam) {
    ...
    async initSellerShow (vModel,param,name) {
